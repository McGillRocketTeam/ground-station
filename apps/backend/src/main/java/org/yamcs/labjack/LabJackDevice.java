 ...
 * needs (open/stream/read/digital IO/watchdog), translating LJM register access into
 * intent-revealing methods.
 *
 * <p>Failures propagate as {@link LJMException} (carrying the numeric LJM error code via {@link
 * LJMException#getError()}); the caller uses {@link #isDisconnectError(int)} to decide whether a
 * failure means "device went away, re-establish the stream". The class is intentionally non-final
 * with overridable methods so tests can subclass it with a fake (no hardware / no Mockito
 * required).
 */
public class LabJackDevice {
   ...
   */
  private static final Set<Integer> DISCONNECT_ERRORS =
      Set.of(
  ...
  public record StreamRead(
      double[] data, int deviceScanBacklog, int ljmScanBacklog, int dummySamples) {}

  /**
   * True if the given LJM error code indicates the device link dropped (stream must be
   * re-established).
   */
  ...
  public void configureAnalogRanges() {
    ...
    log.info(
        "AIN ranges set (AIN0-"
            + (LabJackConfig.AIN_LOW_RANGE_CHANNEL_COUNT - 1)
            + " = ±"
            + LabJackConfig.AIN_LOW_RANGE_V
            + "V, rest = ±"
            + LabJackConfig.AIN_HIGH_RANGE_V
            + "V)");
  }

  /**
   * Arms the T7 hardware watchdog (datasheet §23) to drive <b>all 23 DIO low</b> — without
   * rebooting — if it times out after {@link LabJackConfig#WATCHDOG_TIMEOUT_S} seconds with no
   * qualifying comms. Uses LJM register <i>names</i> so the address map can't drift. Register
   * semantics (per §23):
   *
   * <ul>
   *   <li>{@code DIO_ENABLE=1} — enable the DIO failsafe action
   *   <li>{@code DIO_INHIBIT=0} — bitmask where 1 = protect/skip a line; 0 affects every DIO
   *   <li>{@code DIO_DIRECTION=0x7FFFFF} — bitmask where 1 = drive as output (all 23 lines)
   *   <li>{@code DIO_STATE=0} — level bitmask; all low
   *   <li>{@code RESET_ENABLE=0} — do <b>not</b> reboot the device on timeout (DIO-low only)
   * </ul>
   *
   * The previous code never set DIO_DIRECTION, so the lines stayed inputs and were not actually
   * driven low — fixed here.
   *
   * <p><b>Feeding the timer:</b> §23 "When Using Stream" — spontaneous stream data does NOT reset
   * the watchdog; only a command-response exchange does. The data link's periodic {@code DIO_STATE}
   * read supplies exactly that, so the watchdog stays fed during normal streaming and only trips
   * when the control station goes silent.
   *
  ...
  public void configureWatchdog() {
    ...
    log.info(
        "Watchdog armed: "
            + LabJackConfig.WATCHDOG_TIMEOUT_S
  ...
  public double startStream() {
    ...
    streaming = true;
    log.info(
        "Stream started: requested "
            + LabJackConfig.SCAN_RATE_HZ
            + " Hz, actual "
            + actualScanRate.getValue()
            + " Hz, "
            + LabJackConfig.SCANS_PER_READ
            + " scans/read");
  ...
  /**
   * Reads one batch ({@link LabJackConfig#SCANS_PER_READ} scans). Blocks until the data is
   * buffered. Throws {@link LJMException} on failure (inspect {@link #isDisconnectError(int)});
   * never returns null.
   *
  ...
  public StreamRead readStream() {
    ...
    return new StreamRead(
        data, deviceScanBacklog.getValue(), ljmScanBacklog.getValue(), dummySamples);
  }
