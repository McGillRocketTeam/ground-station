...
public class LabJackDeviceV2 {
  ...
  private static final Set<LJM.Errors> DISCONNECT_ERRORS =
      Set.of(
  ...
  private static final Set<LJM.Errors> RESTART_STREAM_ERRORS =
      Set.of(LJM.Errors.LJM_BUFFER_FULL, LJM.Errors.DIGITAL_AUTO_RECOVERY_ERROR_DETECTED);
  ...
  public static boolean isRestartStreamError(int error) {
    return error == STREAM_SCAN_OVERLAP
        || enumValue(error).map(RESTART_STREAM_ERRORS::contains).orElse(false);
  }

  /**
   * Some stream-read failures recover cleanly by restarting the stream on the existing handle
   * instead of tearing the whole device session down.
   */
  ...
  /**
   * Some command-response reads can time out briefly while the stream is otherwise healthy.
   * Treating that as a full disconnect causes the "few packets, then reconnect loop" behavior.
   */
  ...
  static String discoverVisibleDevices() {
    ...
    for (int i = 0; i < count; i++) {
      ...
      summary
          ...
          .append(", transport=")
          .append(connectionTypeName(connectionTypes[i]))
          .append(", ip=")
          .append(ipAddressToString(ipAddresses[i]))
          .append(" (raw=")
          .append(Integer.toUnsignedString(ipAddresses[i]))
          .append(")");
    }
  ...
  public PollRead readBasicLoop() {
    ...
    byte[] digitalState =
        LabJackPacket.encodeDigitalState((long) values[LabJackConfigV2.NUM_ANALOG_PINS]);
  ...
  public double startStream() {
    ...
    LJM.eStreamStart(
        handle,
        LabJackConfigV2.SCANS_PER_READ,
