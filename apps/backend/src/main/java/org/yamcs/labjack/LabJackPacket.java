...
/**
 * Binary (de)serialisation for the LabJack telemetry packet, matching the XTCE container {@code
 * /LabJackT7/LabJackPacket} in {@code labjack-t7.xml}.
 *
 * <p>Wire layout (big-endian), {@value #PACKET_SIZE} bytes total:
 *
 * <ul>
 *   <li>{@link LabJackConfig#NUM_ANALOG_PINS} IEEE-754 float32 AIN values (AIN0 first)
 *   <li>3 bytes of digital state — 23 DIO bits, MSB of byte 0 = DIO0/FIO0, last bit padding
 * </ul>
 *
...
public final class LabJackPacket {
  ...
  /**
   * Encodes the raw {@code DIO_STATE} register value (bit <i>i</i> = DIO<i>i</i>, bits 0-22) into
   * the 3-byte MSB-first wire format the MDB expects (DIO0 = most-significant bit of byte 0).
   */
