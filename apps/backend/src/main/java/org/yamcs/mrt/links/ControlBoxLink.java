...
public class ControlBoxLink extends AbstractTmDataLink implements MqttTopicHandler {
  ...
  private static final String FLIGHT_COMPUTER_INSTANCE = "launch-canada";
  ...
  private static final Map<Integer, SwitchMapping> SWITCH_PIN_MAP =
      Map.ofEntries(
          ...
          Map.entry(6,  new SwitchMapping("panel_5_switch_1",       -1)),             // F/DOV command only
  ...
  /** Byte offset of the F/DOV control switch in the telemetry packet. */
  private static final int FDOV_OFFSET = 6;
  ...
  private void handleEmergencyStop() {
    ...
    issueFlightComputerCommand("emergency_stop");
  }

  private void handleEmergencyStopClear() {
    log.warn("EMERGENCY STOP CLEARED - issuing emergency_cancel");
    issueFlightComputerCommand("emergency_cancel");
  }

  private void handleArmingKeyChange(boolean armed) {
    String commandName = armed ? "mov_arming" : "mov_disarming";
    log.info("Arming key changed - issuing " + commandName);
    issueFlightComputerCommand(commandName);
  }

  private void handleFdovChange(boolean energized) {
    String commandName = energized ? "fdov_energize" : "fdov_de-energize";
    log.info("F/DOV switch changed - issuing " + commandName);
    issueFlightComputerCommand(commandName);
  }
  ...
  private void issueFlightComputerCommand(String commandName) {
    String url =
        String.format(
            "http://localhost:%d/api/processors/%s/%s/commands/%%2FFlightComputer%%2F%s",
            YAMCS_HTTP_PORT, FLIGHT_COMPUTER_INSTANCE, YAMCS_PROCESSOR, commandName);

    HttpRequest request =
        HttpRequest.newBuilder()
            .uri(URI.create(url))
            .header("Content-Type", "application/json")
            .POST(HttpRequest.BodyPublishers.ofString("{\"args\":{},\"extra\":{}}"))
            .build();
    ...
    httpClient
        ...
        .thenAccept(
            response -> {
              if (response.statusCode() == 200) {
                log.info("Issued FlightComputer command: " + commandName);
              } else {
                log.warn(
                    "Failed to issue FlightComputer command "
                        + commandName
                        + ": HTTP "
                        + response.statusCode()
                        + " - "
        ...
        .exceptionally(
            ex -> {
              log.error(
                  "Error issuing FlightComputer command " + commandName + ": " + ex.getMessage());
              return null;
            });
  }
}
