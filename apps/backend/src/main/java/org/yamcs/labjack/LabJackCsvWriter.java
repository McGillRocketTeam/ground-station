...
/**
 * Writes the full-rate local CSV record of every LabJack scan. This is deliberately independent of the
 * YAMCS publishing rate: even when the realtime/archive path is decimated for IO-latency reasons, the
 * CSV keeps every sample for post-test analysis.
 *
 * <p>Decoding is delegated to {@link LabJackPacket} so the CSV and the wire format can never disagree.
 * IO errors are logged (and disable further writes) rather than thrown, so a disk hiccup can't kill the
 * acquisition thread.
 */
public class LabJackCsvWriter {
    ...
    public LabJackCsvWriter() {
        String name = "labj_" + new SimpleDateFormat("yyyy-MM-dd--HH-mm-ss").format(new Date()) + ".csv";
    ...
    public void open() {
        try {
            ...
            log.error("Could not open LabJack CSV file " + file.getAbsolutePath() + ": " + e.getMessage());
            healthy = false;
        }
    }
    ...
    public void writeRow(long receptionTimeMillis, byte[] packet) {
        ...
        row.append(Instant.ofEpochMilli(receptionTimeMillis)
                .atZone(ZoneId.systemDefault()).toLocalTime().format(TIME_FMT)).append(",");
