...
/**
 * @author Jake
 *         Utility class for LabJack. Contains a bunch of static methods that
 *         provide abstraction
...
public class LabJackUtil {
	 ...
	 * WARNING: using this method to read a digital pin that is currently in write
	 * mode will put the pin in read mode.
	 * Ex: if you previously wrote LOW to DIO1, then read DIO1 with this method,
	 * typically this method will return LOW
	...
	/**
	 * Reads the DIO_STATE on the LabJack. The DIO_STATE register contains the state
	 * of every digital pin
	 * on the LabJack. It is preferred to read this register over reading the
	 * individual pins since reading
	 * individual pins will cause their directionality to be set to input.
	 * The DIO_STATE contains a 32-bit unsigned integer for which the most
	 * significant 9 bits are garbage
	 * (32-9=23! # of digital pins).
	 *
	 * @param deviceHandle device handle of the connected LabJack
	 * @return an array of 3 bytes for which the last element's least significant
	 *         bit is garbage
	 */
	 ...
	 * 
	 * @param deviceHandle device handle of the connected LabJack
	 * @param pinNum       pin number of digital pin to write to (between 0-22
	 *                     inclusive)
