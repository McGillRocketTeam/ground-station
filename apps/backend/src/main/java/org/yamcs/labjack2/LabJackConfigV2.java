...
public final class LabJackConfigV2 {
    ...
    // Stream data does not feed the watchdog. This command-response read does, so once per second is enough.
    ...
    public static void applyOverrides(YConfiguration config) {
        ...
        log.info("LabJackV2 config: scanRate={} Hz, scansPerRead={}, resolutionIndex={}, graphFreq={}, "
    ...
    public static void validateSamplingConfig(double achievedScanRateHz) {
        ...
        if (actualPacketRateHz + TM_PACKET_RATE_TOLERANCE_HZ < MIN_REALTIME_PACKET_RATE_HZ) {
            throw new IllegalArgumentException("LabJackV2 realtime packet rate must be at least "
                    + MIN_REALTIME_PACKET_RATE_HZ + " Hz, got " + actualPacketRateHz
        ...
        if (Math.abs(actualPacketRateHz - TM_PACKET_RATE_HZ) > TM_PACKET_RATE_TOLERANCE_HZ) {
            throw new IllegalArgumentException("LabJackV2 packet path must run at " + TM_PACKET_RATE_HZ
