...
/**
 * Central place for every tunable that affects LabJack acquisition, the YAMCS publishing rate and
 * the watchdog. Values are plain {@code static} fields so they can be changed with a one-line edit
 * and a rebuild (per the DAQ "adjust via a static variable" workflow), and the most useful ones can
 * also be overridden at runtime from the link's {@code args:} block in {@code
 * yamcs.ground_station.yaml} via {@link #applyOverrides(YConfiguration)} — no recompile needed on
 * the GS computer.
 *
 * <p>None of these touch sensor calibration; calibration lives in the MDB ({@code labjack-t7.xml})
 * and is owned by the DAQ team.
 */
public final class LabJackConfig {
  ...
  /**
   * Stream scan rate in scans/channel/second. MVP target is 300 Hz; the performance test pushes
   * this to 500 Hz. T7 aggregate limit is 100 kSamples/s, so 14 channels * 500 Hz = 7 kS/s is well
   * within range (needs {@link #STREAM_RESOLUTION_INDEX} 0 or 1).
   */
   ...
   * report calls out: larger = fewer, bigger batches (multiple data points land in YAMCS at the
   * same timestamp); smaller = lower latency. eStreamRead blocks until this many scans are
   * buffered, so at 300 Hz a value of 12 yields 25 reads/s (~40 ms batches).
   */
   ...
   * therefore the default packet-saving path). The default 300 Hz / 12 = 25 Hz. Raising this
   * throttles the websocket/UI without affecting the CSV (always full rate) or — when {@link
   * #ARCHIVE_FULL_RATE} is on — the YAMCS archive.
   */
   ...
   * performance test. The second stream must be declared in streamConfig — see
   * yamcs.ground_station.yaml.
   */
  ...
  /**
   * Optionally override the runtime-tunable fields from the link config. Unknown keys are ignored,
   * so an empty {@code args:} block leaves all defaults in place. Called from {@code
   * LabJackDataLink.init}.
   */
  public static void applyOverrides(YConfiguration config) {
    ...
    log.info(
        "LabJack config: scanRate={} Hz, scansPerRead={}, resolutionIndex={}, graphFreq={}, "
  ...
  public static void validateSamplingConfig(double achievedScanRateHz) {
    ...
    //     throw new IllegalArgumentException("LabJack requires scansPerRead == graphFreq so packets
    // are"
    //             + " emitted one per stream read at a stable rate; got scansPerRead=" +
    // SCANS_PER_READ
