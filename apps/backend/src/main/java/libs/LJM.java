...
/**
 * LJM is the wrapper class to the LJM library's functions and constants. Refer to the LabJackM.h
 * header file or online User's Guide for functions and constants documentation:<br>
 * <br>
 * &nbsp;&nbsp;&nbsp;&nbsp;<a
 * href="https://labjack.com/support/software/api/ljm">https://labjack.com/support/software/api/ljm</a>
 * <br>
 *
 * <p>LJM library C to Java differences:
 *
 * <ul>
 *   <li>C functions are implemented in the LJM class as static methods. The function name's "LJM_"
 *       prefix have been removed and the first letter have been changed to lowercase.
 *   <li>C constants can be found in the LJM.Constants class. The constant name's "LJM_" prefix have
 *       been removed.
 *   <li>C error code constants can be found in the LJM.Errors enum. The constant name's "LJME_"
 *       prefix have been removed.
 *   <li>C function parameter names have had the first letter changed to lowercase.
 *   <li>If the wrapper method detects an error it will throw a LJMException exception, setting the
 *       error and errorAddress values, and exception message.
 *   <li>C parameters that are pass by reference are implemented in Java as arrays, or JNA classes
 *       IntByReference, DoubleByReference, or LongByReference for single value references.
 *   <li>C string parameters are implemented in Java as a String and pass by reference strings are a
 *       JNA class Pointer.<br>
 *   <li>When using a JNA Pointer for C strings, the JNA Memory class needs to be used to construct
 *       the pointer and allocate memory. For example:<br>
 *       <br>
 *       &nbsp;&nbsp;&nbsp;&nbsp;Pointer stringPtr = new Memory(100);&nbsp;&nbsp;//Allocates 100
 *       bytes of memory which is a 100 character C string.
 * </ul>
 *
 * <p>Version History
 *
 * <ul>
 *   <li>1.23<br>
 *       &nbsp;&nbsp;&nbsp;&nbsp;- Updated functions, constants and error codes to LJM v1.23.
 *   <li>1.17<br>
 *       &nbsp;&nbsp;&nbsp;&nbsp;- Updated functions, constants and error codes to LJM v1.17.
 *   <li>1.16<br>
 *       &nbsp;&nbsp;&nbsp;&nbsp;- Updated functions, constants and error codes to LJM v1.16.<br>
 ...
 *   <li>0.90<br>
 *       &nbsp;&nbsp;&nbsp;&nbsp;- Initial release tested with LJM v1.1.1. Windows support only.
 * </ul>
 *
...
public class LJM {
  public interface LabJackM extends Library {
    ...
    int LJM_ListAllExtended(
        int DeviceType,
        int ConnectionType,
        int NumAddresses,
        final int[] aAddresses,
        final int[] aNumRegs,
        int MaxNumFound,
        IntByReference NumFound,
        int[] aDeviceTypes,
    ...
    int LJM_Open(
        int DeviceType, int ConnectionType, final String Identifier, IntByReference Handle);

    int LJM_GetHandleInfo(
        int Handle,
        IntByReference DeviceType,
        IntByReference ConnectionType,
        IntByReference SerialNumber,
        IntByReference IPAddress,
        IntByReference Port,
    ...
    int LJM_eReadAddress(int Handle, int Address, int Type, DoubleByReference Value);

    int LJM_eWriteName(int Handle, final String Name, double Value);

    int LJM_eReadName(int Handle, final String Name, DoubleByReference Value);
    ...
    int LJM_eReadAddressByteArray(
        int Handle, int Address, int NumBytes, byte[] aBytes, IntByReference ErrorAddress);

    int LJM_eReadNameByteArray(
        int Handle, final String Name, int NumBytes, byte[] aBytes, IntByReference ErrorAddress);

    int LJM_eWriteAddressByteArray(
        int Handle, int Address, int NumBytes, byte[] aBytes, IntByReference ErrorAddress);

    int LJM_eWriteNameByteArray(
        int Handle, final String Name, int NumBytes, byte[] aBytes, IntByReference ErrorAddress);
    ...
    int LJM_eWriteNameString(int Handle, final String Name, final String string);

    int LJM_eWriteAddressString(int Handle, int Address, final String string);
    ...
    int LJM_GetStreamTCPReceiveBufferStatus(
        int Handle,
        IntByReference ReceiveBufferBytesSize,
        IntByReference ReceiveBufferBytesBacklog);

    int LJM_InitializeAperiodicStreamOut(
        int Handle, int StreamOutIndex, int TargetAddr, double ScanRate);

    int LJM_WriteAperiodicStreamOut(
        int Handle,
        int StreamOutIndex,
        int NumValues,
    ...
    int LJM_AddressesToMBFB(
        ...
        final int[] aNumValues,
        final double[] aValues,
        IntByReference NumFrames,
        byte[] aMBFBCommand);

    int LJM_MBFBComm(int Handle, byte UnitID, byte[] aMBFB, IntByReference ErrorAddress);

    int LJM_UpdateValues(
        byte[] aMBFBResponse,
        final int[] aTypes,
        final int[] aWrites,
        final int[] aNumValues,
    ...
    int LJM_NamesToAddresses(int NumFrames, final String[] aNames, int[] aAddresses, int[] aTypes);

    int LJM_NameToAddress(final String Name, IntByReference Address, IntByReference Type);

    int LJM_AddressesToTypes(int NumAddresses, int[] aAddresses, int[] aTypes);

    int LJM_AddressToType(int Address, IntByReference Type);

    int LJM_LookupConstantValue(
        final String Scope, final String ConstantName, DoubleByReference ConstantValue);

    int LJM_LookupConstantName(final String Scope, double ConstantValue, Pointer ConstantName);
    ...
    int LJM_TCVoltsToTemp(int TCType, double TCVolts, double CJTempK, DoubleByReference pTCTempK);

    void LJM_FLOAT32ToByteArray(
        final float[] aFLOAT32, int RegisterOffset, int NumFLOAT32, byte[] aBytes);

    void LJM_ByteArrayToFLOAT32(
        final byte[] aBytes, int RegisterOffset, int NumFLOAT32, float[] aFLOAT32);

    void LJM_UINT16ToByteArray(
        final short[] aUINT16, int RegisterOffset, int NumUINT16, byte[] aBytes);

    void LJM_ByteArrayToUINT16(
        final byte[] aBytes, int RegisterOffset, int NumUINT16, short[] aUINT16);

    void LJM_UINT32ToByteArray(
        final int[] aUINT32, int RegisterOffset, int NumUINT32, byte[] aBytes);

    void LJM_ByteArrayToUINT32(
        final byte[] aBytes, int RegisterOffset, int NumUINT32, int[] aUINT32);

    void LJM_INT32ToByteArray(final int[] aINT32, int RegisterOffset, int NumINT32, byte[] aBytes);

    void LJM_ByteArrayToINT32(final byte[] aBytes, int RegisterOffset, int NumINT32, int[] aINT32);
    ...
    void LJM_GetHostTick32Bit(IntByReference TickUpper, IntByReference TickLower);

    int LJM_StartInterval(int IntervalHandle, int Microseconds);

    int LJM_WaitForNextInterval(int IntervalHandle, IntByReference SkippedIntervals);

    int LJM_CleanInterval(int IntervalHandle);

    int LJM_WriteLibraryConfigS(final String Parameter, double Value);

    int LJM_WriteLibraryConfigStringS(final String Parameter, final String string);

    int LJM_ReadLibraryConfigS(final String Parameter, DoubleByReference Value);

    int LJM_ReadLibraryConfigStringS(final String Parameter, Pointer string);

    int LJM_LoadConfigurationFile(final String FileName);

    int LJM_GetSpecificIPsInfo(IntByReference InfoHandle, PointerByReference Info);
  ...
  /**
   * Constants is a class containing the constants from the LJM library. The "LJM_" prefix have been
   * removed from the original names.
   */
  public class Constants {
    ...
    public static final String ETHERNET_SEND_RECEIVE_TIMEOUT_MS =
    ...
    public static final String ALLOWS_AUTO_MULTIPLE_FEEDBACKS =
        "LJM_ALLOWS_AUTO_MULTIPLE_FEEDBACKS";
    public static final String ALLOWS_AUTO_CONDENSE_ADDRESSES =
    ...
    public static final String AUTO_RECONNECT_STICKY_CONNECTION =
    ...
    public static final String STREAM_DIGITAL_AUTO_RECOVERY_ERROR_DETECTION_DISABLED =
    ...
    public static final String STREAM_TCP_RECEIVE_BUFFER_SIZE =
    ...
    public static final String RETRY_ON_TRANSACTION_ID_MISMATCH =
    ...
    /**
     * @deprecated Replaced by {@link #ETHERNET_UDP_PORT} and {@link #WIFI_UDP_PORT}
     */
    @Deprecated public static final int UDP_PORT = 52362;

    /**
     * @deprecated Maximum packet size should instead be read with {@link #getHandleInfo}
     */
     ...
     */
    @Deprecated
    public static final String SPECIAL_ADDRESSES_STATUS = "LJM_SPECIAL_ADDRESSES_STATUS";
  ...
  /**
   * Errors is an enum containing the error constants from the LJM library. The "LJME_" prefix have
   * been removed from the original names.
   */
  public enum Errors {
    NOERROR(0),
    WARNINGS_BEGIN(200),
  ...
  public static int listAllS(
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_ListAllS(
            deviceType,
            connectionType,
            numFound,
            aDeviceTypes,
  ...
  public static int listAllExtended(
      int deviceType,
      int connectionType,
      int numAddresses,
      final int[] aAddresses,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_ListAllExtended(
            deviceType,
            connectionType,
            numAddresses,
            aAddresses,
  ...
  public static int openS(
      ...
      IntByReference handle) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_OpenS(deviceType, connectionType, identifier, handle);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int open(
      int deviceType, int connectionType, final String identifier, IntByReference handle) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_Open(deviceType, connectionType, identifier, handle);
    LJM.handleError(error);
    return error;
  }

  public static int getHandleInfo(
      int handle,
      IntByReference deviceType,
      IntByReference connectionType,
      IntByReference serialNumber,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_GetHandleInfo(
            handle, deviceType, connectionType, serialNumber, ipAddress, port, maxBytesPerMB);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int eWriteAddress(int handle, int address, int type, double value) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eWriteAddress(handle, address, type, value);
    LJM.handleError(error);
    return error;
  }

  public static int eReadAddress(int handle, int address, int type, DoubleByReference value) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eReadAddress(handle, address, type, value);
    LJM.handleError(error);
    return error;
  }

  public static int eWriteName(int handle, final String name, double value) {
    int error = 0;
  ...
  public static int eReadName(int handle, final String name, DoubleByReference value) {
    int error = 0;
  ...
  public static int eReadAddresses(
      int handle,
      int numFrames,
      final int[] aAddresses,
      final int[] aTypes,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eReadAddresses(
            handle, numFrames, aAddresses, aTypes, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eReadNames(
      int handle,
      int numFrames,
      final String[] aNames,
      double[] aValues,
      IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eReadNames(handle, numFrames, aNames, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eWriteAddresses(
      int handle,
      int numFrames,
      final int[] aAddresses,
      final int[] aTypes,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eWriteAddresses(
            handle, numFrames, aAddresses, aTypes, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eWriteNames(
      int handle,
      int numFrames,
      final String[] aNames,
      final double[] aValues,
      IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eWriteNames(handle, numFrames, aNames, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eReadAddressArray(
      int handle,
      int address,
      int type,
      int numValues,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eReadAddressArray(
            handle, address, type, numValues, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eReadNameArray(
      int handle, final String name, int numValues, double[] aValues, IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eReadNameArray(handle, name, numValues, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eWriteAddressArray(
      int handle,
      int address,
      int type,
      int numValues,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eWriteAddressArray(
            handle, address, type, numValues, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eWriteNameArray(
      int handle,
      final String name,
      int numValues,
      final double[] aValues,
      IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eWriteNameArray(handle, name, numValues, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eReadAddressByteArray(
      int handle, int address, int numBytes, byte[] aBytes, IntByReference errorAddress) {
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eReadAddressByteArray(
            handle, address, numBytes, aBytes, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eReadNameByteArray(
      int handle, final String name, int numBytes, byte[] aBytes, IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eReadNameByteArray(handle, name, numBytes, aBytes, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eWriteAddressByteArray(
      int handle, int address, int numBytes, final byte[] aBytes, IntByReference errorAddress) {
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eWriteAddressByteArray(
            handle, address, numBytes, aBytes, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eWriteNameByteArray(
      int handle,
      final String name,
      int numBytes,
      final byte[] aBytes,
      IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eWriteNameByteArray(handle, name, numBytes, aBytes, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eAddresses(
      int handle,
      int numFrames,
      final int[] aAddresses,
      final int[] aTypes,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eAddresses(
            handle, numFrames, aAddresses, aTypes, aWrites, aNumValues, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }
  ...
  public static int eNames(
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eNames(
            handle, numFrames, aNames, aWrites, aNumValues, aValues, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }

  public static int eReadNameString(int handle, final String name, Pointer string) {
    int error = 0;
  ...
  public static int eReadAddressString(int handle, int address, Pointer string) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eReadAddressString(handle, address, string);
    LJM.handleError(error);
    return error;
  }

  public static int eWriteNameString(int handle, final String name, final String string) {
    int error = 0;
  ...
  public static int eWriteAddressString(int handle, int address, final String string) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eWriteAddressString(handle, address, string);
    LJM.handleError(error);
    return error;
  }

  public static int eStreamStart(
      int handle,
      int scansPerRead,
      int numAddresses,
      final int[] aScanList,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_eStreamStart(handle, scansPerRead, numAddresses, aScanList, scanRate);
    LJM.handleError(error);
    return error;
  }

  public static int eStreamRead(
      int handle, double[] aData, IntByReference deviceScanBacklog, IntByReference ljmScanBacklog) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_eStreamRead(handle, aData, deviceScanBacklog, ljmScanBacklog);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int streamBurst(
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_StreamBurst(
            handle, numAddresses, aScanList, scanRate, numScans, aData);
    LJM.handleError(error);
    return error;
  }

  public static int getStreamTCPReceiveBufferStatus(
      int handle, IntByReference receiveBufferBytesSize, IntByReference receiveBufferBytesBacklog) {
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_GetStreamTCPReceiveBufferStatus(
            handle, receiveBufferBytesSize, receiveBufferBytesBacklog);
    LJM.handleError(error);
    return error;
  }

  public static int initializeAperiodicStreamOut(
      int handle, int streamOutIndex, int targetAddr, double scanRate) {
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_InitializeAperiodicStreamOut(
            handle, streamOutIndex, targetAddr, scanRate);
    LJM.handleError(error);
    return error;
  }

  public static int writeAperiodicStreamOut(
      int handle,
      int streamOutIndex,
      int numValues,
      final double[] aWriteData,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_WriteAperiodicStreamOut(
            handle, streamOutIndex, numValues, aWriteData, ljmBufferStatus);
    LJM.handleError(error);
    return error;
  }

  public static int periodicStreamOut(
      int handle,
      int streamOutIndex,
      int targetAddr,
      double scanRate,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_PeriodicStreamOut(
            handle, streamOutIndex, targetAddr, scanRate, numValues, aWriteData);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int addressesToMBFB(
      int maxBytesPerMBFB,
      final int[] aAddresses,
      final int[] aTypes,
      final int[] aWrites,
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_AddressesToMBFB(
            maxBytesPerMBFB,
            aAddresses,
            aTypes,
            aWrites,
  ...
  public static int mbfbComm(int handle, byte unitID, byte[] aMBFB, IntByReference errorAddress) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_MBFBComm(handle, unitID, aMBFB, errorAddress);
    LJM.handleError(error, errorAddress.getValue());
    return error;
  }
  ...
  public static int updateValues(
    ...
    int error = 0;
    error =
        LabJackM.INSTANCE.LJM_UpdateValues(
            aMBFBResponse, aTypes, aWrites, aNumValues, numFrames, aValues);
    LJM.handleError(error);
    return error;
  }

  public static int namesToAddresses(
      int numFrames, final String[] aNames, int[] aAddresses, int[] aTypes) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_NamesToAddresses(numFrames, aNames, aAddresses, aTypes);
    LJM.handleError(error);
    return error;
  }

  public static int nameToAddress(final String name, IntByReference address, IntByReference type) {
    int error = 0;
  ...
  public static int addressesToTypes(int numAddresses, int[] aAddresses, int[] aTypes) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_AddressesToTypes(numAddresses, aAddresses, aTypes);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int lookupConstantValue(
      final String scope, final String constantName, DoubleByReference constantValue) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_LookupConstantValue(scope, constantName, constantValue);
    LJM.handleError(error);
    return error;
  }

  public static int lookupConstantName(
      final String scope, double constantValue, Pointer constantName) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_LookupConstantName(scope, constantValue, constantName);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int tcVoltsToTemp(
      int tcType, double tcVolts, double cjTempK, DoubleByReference pTCTempK) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_TCVoltsToTemp(tcType, tcVolts, cjTempK, pTCTempK);
    LJM.handleError(error);
    return error;
  }

  public static void float32ToByteArray(
      final float[] aFLOAT32, int registerOffset, int numFLOAT32, byte[] aBytes) {
    LabJackM.INSTANCE.LJM_FLOAT32ToByteArray(aFLOAT32, registerOffset, numFLOAT32, aBytes);
  }

  public static void byteArrayToFLOAT32(
      final byte[] aBytes, int registerOffset, int numFLOAT32, float[] aFLOAT32) {
    LabJackM.INSTANCE.LJM_ByteArrayToFLOAT32(aBytes, registerOffset, numFLOAT32, aFLOAT32);
  }

  public static void uint16ToByteArray(
      final short[] aUINT16, int registerOffset, int numUINT16, byte[] aBytes) {
    LabJackM.INSTANCE.LJM_UINT16ToByteArray(aUINT16, registerOffset, numUINT16, aBytes);
  }

  public static void byteArrayToUINT16(
      final byte[] aBytes, int registerOffset, int numUINT16, short[] aUINT16) {
    LabJackM.INSTANCE.LJM_ByteArrayToUINT16(aBytes, registerOffset, numUINT16, aUINT16);
  }

  public static void uint32ToByteArray(
      final int[] aUINT32, int registerOffset, int numUINT32, byte[] aBytes) {
    LabJackM.INSTANCE.LJM_UINT32ToByteArray(aUINT32, registerOffset, numUINT32, aBytes);
  }

  public static void byteArrayToUINT32(
      final byte[] aBytes, int registerOffset, int numUINT32, int[] aUINT32) {
    LabJackM.INSTANCE.LJM_ByteArrayToUINT32(aBytes, registerOffset, numUINT32, aUINT32);
  }

  public static void int32ToByteArray(
      final int[] aINT32, int registerOffset, int numINT32, byte[] aBytes) {
    LabJackM.INSTANCE.LJM_INT32ToByteArray(aINT32, registerOffset, numINT32, aBytes);
  }

  public static void byteArrayToINT32(
      final byte[] aBytes, int registerOffset, int numINT32, int[] aINT32) {
  ...
  public static int ipToNumber(final String ipv4String, IntByReference number) {
    int error = 0;
  ...
  public static int macToNumber(final String macString, LongByReference number) {
    int error = 0;
  ...
  public static void getHostTick32Bit(IntByReference TickUpper, IntByReference TickLower) {
  ...
  public static int startInterval(int intervalHandle, int microseconds) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_StartInterval(intervalHandle, microseconds);
    LJM.handleError(error);
    return error;
  }

  public static int waitForNextInterval(int intervalHandle, IntByReference skippedIntervals) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_WaitForNextInterval(intervalHandle, skippedIntervals);
    LJM.handleError(error);
    return error;
  }
  ...
  public static int writeLibraryConfigS(final String parameter, double value) {
    int error = 0;
  ...
  public static int writeLibraryConfigStringS(final String parameter, final String string) {
    int error = 0;
    error = LabJackM.INSTANCE.LJM_WriteLibraryConfigStringS(parameter, string);
    LJM.handleError(error);
    return error;
  }

  public static int readLibraryConfigS(final String parameter, DoubleByReference value) {
    int error = 0;
  ...
  public static int readLibraryConfigStringS(final String parameter, Pointer string) {
    int error = 0;
  ...
  public static int getSpecificIPsInfo(IntByReference infoHandle, PointerByReference info) {
    int error = 0;
