        ...
        lookup: () =>
          Effect.acquireRelease(
            Effect.gen(function* () {
              ...
              const priorCommands =
                ("commands" in priorCommandResponse ? priorCommandResponse.commands : undefined) ??
                ("entry" in priorCommandResponse ? priorCommandResponse.entry : undefined) ??
                [];

              let initialEntriesById = new Map<string, typeof StreamingCommandHisotryEntry.Type>();
